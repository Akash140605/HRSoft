...
export default function ScannerPanel() {
  ...
  const selectedEntries = useMemo(() => {
    ...
    return src.filter((e) => entryDateOf(e) === dateKey(selectedDate));
  }, [state.entries, selectedDate]);

  const hallSummary = useMemo(() => {
    const hallMap = new Map((state.halls || []).map((h) => [String(h.id).trim(), h]));
    ...
    const entryByHallShift = new Map();
    selectedEntries.forEach((e) => {
      const key = `${entryHallIdOf(e)}__${entryShiftOf(e)}`;
      const arr = entryByHallShift.get(key) || [];
      arr.push(e);
      entryByHallShift.set(key, arr);
    });
    ...
    return Array.from(hallMap.values()).map((hall) => {
      ...
      (state.SHIFT_OPTIONS || []).forEach((shift) => {
        const rosterRows =
          rosterByHallShift.get(`${String(hall.id).trim()}__${shift.code}`) || [];
        const capacity = rosterRows.filter((emp) => !isWeekOff(emp, selectedDate)).length;
        const usedCount =
          (entryByHallShift.get(`${String(hall.id).trim()}__${shift.code}`) || []).length;
      ...
      const rosterAssigned = currentRoster.filter(
  ...
  const recentRows = useMemo(() => {
    ...
    return selectedEntries.filter((e) => {
      const text = `${e.name || ""} ${e.code || ""} ${e.designation || ""} ${e.hallName || e.hall_name || ""} ${e.source || e.type || ""} ${e.overrideReason || e.override_reason || ""}`.toLowerCase();
  ...
  const onProcess = async (inputCode) => {
    ...
    try {
      ...
      const alreadyScanned =
        res?.alreadyScanned || text.includes("already scanned") || text.includes("already");
  ...
  const HallCard = ({ h, mobile = false }) => {
    const percent =
      h.effectiveCapacity > 0
        ? Math.min(100, Math.round((h.used / h.effectiveCapacity) * 100))
        : 0;
    ...
    return (
      <div className={mobile ? "rounded border border-slate-200 p-2" : "border-2 border-slate-300 p-4"}>
        <button
          type="button"
          onClick={() => setExpandedHall(expanded ? null : h.id)}
          className="flex w-full items-center justify-between text-left"
        >
          <div>
            <div className={mobile ? "text-xs font-semibold text-slate-900" : "font-bold text-slate-900"}>
              {h.name}
            </div>
        ...
        <div className={mobile ? "mt-2 h-2 overflow-hidden rounded bg-slate-100" : "mt-3 h-2 overflow-hidden border-2 border-slate-300 bg-white"}>
          <div
            className={`${h.full ? "bg-[#E0222A]" : "bg-slate-700"} h-2`}
            style={{ width: `${percent}%` }}
          />
        </div>
        ...
        {expanded && (
          <div className="mt-3 space-y-2">
            {(state.SHIFT_OPTIONS || []).map((shift) => {
              ...
              const shiftPercent =
                info.capacity > 0
                  ? Math.min(100, Math.round((info.used / Math.max(info.capacity, 1)) * 100))
                  : 0;
              ...
              return (
                <div key={`${h.id}_${shift.code}`} className="rounded border border-slate-200 bg-slate-50 p-2">
                  ...
                  <div className="mt-2 h-2 overflow-hidden rounded bg-white ring-1 ring-slate-200">
                    <div
                      className={`h-2 ${info.capacity > 0 && info.used >= info.capacity ? "bg-[#E0222A]" : "bg-emerald-600"}`}
          ...
          <div className="p-4 sm:p-5">
              ...
              <div className="border-2 border-slate-300 bg-white p-5">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <input
                    ...
                    className="border-2 border-slate-300 px-3 py-2 text-sm outline-none"
                  />
                </div>
                <div className="mt-4 space-y-3">
                  {hallSummary.map((h) => (
                    <HallCard key={h.id} h={h} />
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="mt-5 border-2 border-slate-300 bg-white p-5">
              ...
              <div className="mt-4 space-y-2">
                {recentRows.map((row) => (
                  <div
                    key={row.id || `${row.code}_${entryDateOf(row)}_${row.time || ""}`}
                    className="flex flex-col gap-1 border border-slate-200 p-3 text-sm sm:flex-row sm:items-center sm:justify-between"
                  >
                    <div>
                    ...
                    <div className="text-xs text-slate-500">{entryDateOf(row)}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            {isHR && pendingHrMove && (
              <div className="mt-3 space-y-2 rounded border border-amber-300 bg-amber-50 p-2">
                <div className="text-[11px] font-semibold text-amber-800">
                  Hall select karo, phir Move Hall dabao.
                </div>
                <select
                  className="w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm outline-none"
                  value={selectedHall}
                  onChange={(e) => setSelectedHall(e.target.value)}
                >
                  {state.halls.map((h) => (
                ...
                />
                <button
                  className="w-full rounded bg-[#E0222A] px-3 py-2 text-sm font-semibold text-white disabled:opacity-50"
                  type="button"
                  onClick={onMove}
                  disabled={busy || !selectedHall}
                >
          ...
          <div className="mx-2 mt-2 rounded border border-slate-200 bg-white p-2">
            <div className="mb-2 flex items-center justify-between">
              ...
              <input
                ...
                value={selectedDate}
                onChange={(e) => setSelectedDate(e.target.value)}
                className="border border-slate-300 px-2 py-1 text-xs outline-none"
              />
            </div>
            <div className="mt-2 space-y-2">
              {hallSummary.map((h) => (
                <HallCard key={h.id} h={h} mobile />
              ))}
            </div>
          </div>
