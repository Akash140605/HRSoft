  ...
  const deleteSelectedEmployees = async () => {
    ...
    try {
      ...
      if (successCount === targetIds.length) {
        setMessage(`✅ ${successCount} employees deleted successfully!`);
      } else {
        setMessage(`⚠️ ${successCount} of ${targetIds.length} employees deleted.`);
      }
    } catch (error) {
  ...
  const removeRow = async (id) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setMessage('Delete Error: ' + response.error);
      }
    } catch (error) {
...
const importCsv = async (file) => {
  ...
  const stats = useMemo(() => {
    ...
    state.employees.forEach((e) =>
      byHall.set(e.hallId, (byHall.get(e.hallId) || 0) + 1)
    );
